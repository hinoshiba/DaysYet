...
- 残りは今日を含む選択日数。残り割合は「今日を含む残りの選択日数 ÷ 全選択日数」。実際の学習実績・完了記録ではない。
...
すべて架空の学習予定。2026-09-09のUIレビュー時点の記録で、ストア提出用の画像ではありません。プレビュー画像の割合は残量表示への変更前のものです。現在の残り割合とバーは[最新のストア画像](../AppStore/README.md)で確認できます。
