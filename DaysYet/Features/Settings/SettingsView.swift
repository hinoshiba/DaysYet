...
private struct PrivacyView: View {
    var body: some View {
        List {
            Section {
                Text(L10n.text(
                    "本アプリは、生年月日・目標年齢・活動時間とラベル・曜日・大切な日の起算日と目標日時・表示設定を、アプリとウィジェットの共有領域に保存します。データを外部へ送信せず、追跡・分析・広告を行いません。設定画面からいつでも全データを消去できます。",
                    "This app stores your birth date, target age, activity hours with labels and weekdays, milestone start and target dates, and display choices in the private area shared by the app and its widget. It sends nothing off device and performs no tracking, analytics, or advertising. You can delete all data from Settings at any time."
                ))
            }
