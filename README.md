...
今週、今月、今年、勤務時間、健康でいたい年齢、大切な日から選び、時間の残りと進み具合を静かに見渡すiPhone / iPad / Macアプリです。iPhone / iPadではホーム画面に3つ、ロック画面に1つを表示。Macでは画面端に常駐する3つの時間から、必要な詳細を開けます。3つの表示モードと4つのカラーテーマを選べます。
...
- 今週、今月、今年、勤務時間、本人が設定する健康年齢の目標、起算日を指定できる任意の目標日時
- 週の始まりは端末のカレンダー設定に従うか、日曜〜土曜のいずれかを指定可能
- 勤務の開始・終了時刻を設定し、毎日の終了までをカウントダウン。日をまたぐ勤務にも対応
...
「勤務時間」は表示する項目から任意で選べます。アプリで開始・終了時刻（初期値は9:00〜18:00）を設定すると、端末の現地時刻で毎日繰り返します。日中の勤務では開始前に「開始前」、勤務中に残り時間、終了後に「勤務終了」を表示します。22:00〜翌6:00のように日をまたぐ設定もでき、開始と終了を同じ時刻にすると24時間として扱います。勤務時刻は各端末に保存され、他の端末には同期されません。
