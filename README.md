...
今週、今月、今年、活動時間、健康でいたい年齢、大切な日から選び、時間の残りと進み具合を静かに見渡すiPhone / iPad / Macアプリです。iPhone / iPadではホーム画面に3つ、ロック画面に1つを表示。Macでは画面端に常駐する時間から、必要な詳細を開けます。標準は3つで、左右の表示では設定から円を追加できます。3つの表示モードと4つのカラーテーマを選べます。
...
- Macの画面端に常駐する、細い黒いパネル。左右には中央に経過割合を表示する円形メーター。標準は3つで、設定から重複しない時間を追加して最大7つまで表示し、3つまで減らせる
