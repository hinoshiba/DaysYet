...
The iPhone / iPad `release_notes.txt` files describe the 0.1.3 update from released 0.1.2 (4): daily Work hours, overnight schedules, and selectable week starts across the app and Widgets. The Mac `release_notes.txt` files are empty because `initial_release: true`; a first platform version has no What’s New field. The release owner separately authorized the iOS 0.1.3 update on 2026-09-06; its App Store Connect version has been created and Japanese/English update text and review notes are saved. The candidate build and refreshed screenshots must still be attached before submission.
...
The four existing images in each locale/device folder show: (1) progress bars, (2) time left with elapsed percentage and bars, (3) the timeline library, and (4) on-device privacy and OSS information. They must be refreshed from the candidate: the current settings image still shows version 0.1.1 (2), and the library predates Work hours and the week-start setting. Use fictional dates and schedules. A successful dimension check does not establish that an image matches the current app.
...
On 2026-09-06, **Submit for Review** succeeded for macOS 0.1.2, build 3. App Store Connect confirmed one submitted item and shows **Waiting for Review** (「審査待ち」), with no draft remaining. Only the Mac version was submitted. The public Mac website update is deployed and verified, and the existing automatic-release-after-approval setting remains selected. Apple has not yet approved the Mac release.
